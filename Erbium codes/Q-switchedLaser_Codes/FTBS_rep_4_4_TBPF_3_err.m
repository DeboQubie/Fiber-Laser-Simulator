...
%% EDFA parameters
EDFA_N0 = 2.65e25;                               % Doping concentration
...
L = 14.8;                                        % length of the cavity
l1 = (L-EDFA_L)/6;                              % length of the fibre section between 2 components
...
cou_l = 3.3/4.343;
c_out = 3.3/4.343;                                % insertion loss of 3-dB coupler
aom_l = l_off/(al*4.343);                                % insertion loss of AOM
tsw = 150e-9;                                    % AOM switching time
wdm_l = 1.6/4.343;                             % insertion loss of WDM

TBPF = 2*ones(1,length(EDFA_lam_s));
% lam_c = 1540e-9;

% TBPF(abs(EDFA_lam_s-lam_c)<1e-15)=2.0;
% TBPF(abs(EDFA_lam_s-lam_c)>0&abs(EDFA_lam_s-lam_c)<=0.5e-9)=5;
% TBPF(abs(EDFA_lam_s-lam_c)>0.5e-9&abs(EDFA_lam_s-lam_c)<=1e-9)=10;
TBPF = TBPF/4.343;
...
N = 175;         % No. of space steps over EDFA
...
bs(:,N+4*nz-(al/2):N+4*nz+(al/2)-1) = -ones(p,al)*aom_l/dz;                  % AOM position
...
%     temps(:,2:Nz) = (1+v*dt.*bs(:,2:Nz) - R).*Ps(:,2:Nz) + R*Ps(:,1:Nz-1);
    ...
    if(n*dt < tsw)
        bs(:,N+4*nz-(al/2):N+4*nz+(al/2)-1) = -ones(p,al)*AOM(n*dt,(l_off/al),(l_on/al),tsw)/(dz*4.343);
    end
    if(n*dt>Th-tsw)
        bs(:,N+4*nz-(al/2):N+4*nz+(al/2)-1) = -ones(p,al)*AOM(n*dt-(Th-tsw),(l_on/al),(l_off/al),tsw)/(dz*4.343);
    end
